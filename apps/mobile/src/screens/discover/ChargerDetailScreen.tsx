...
function PhotoViewer({ photos, index, onChangeIndex, onClose }: { photos: string[]; index: number; onChangeIndex: (i: number) => void; onClose: () => void }) {
  ...
  // Direct 1:1 tracking while pinching — no easing on the way up, the
  // clamp just stops it from going past the min/max. Only on release does
  // withSpring take over, snapping back with a slight overshoot rather
  // than a persistent zoom (momentary, not sticky, like the web version).
  const pinchGesture = Gesture.Pinch()
    .onUpdate((e) => {
      scale.value = Math.min(Math.max(e.scale, MIN_ZOOM), MAX_ZOOM);
    })
    .onEnd(() => {
        ...
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 16 }}>
          ...
          <GestureDetector gesture={pinchGesture}>
            <Animated.Image
              ...
              resizeMode="contain"
              style={[{ width: "100%", height: "100%", borderRadius: radii.xl, borderWidth: 1, borderColor: tokens.hair }, animatedImageStyle]}
            />
          </GestureDetector>
          {photos.length > 1 && (
            <Pressable
